...
function getArtistByIndex( yourArray, index) {
  return "the artist at index " + index + " is " + yourArray[index].name
...
function listOfNames(yourArray) {
 const newArr = []
 for (let i=0; i< yourArray.length; i++){
  newArr.push(yourArray[i].name)
 }
 return newArr
}
...
function addArtist(array,aName,aYear,aGenre,aNationaity,aBio ) {
  array.push({name: aName, years: aYear, genre: aGenre, nationality: aNationaity, bio: aBio, })
  return array
}
...
function artistByCountry(array, aNationality){
  let newArr = []
 for (let i = 0; i<array.length; i++){
  if (array[i].nationality === aNationality){
    newArr.push(array[i].name)
  }
 }
 return newArr
